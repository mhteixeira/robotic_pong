...
if __name__ == "__main__":
	...
	# https://github.com/ravirajsinh45/connect_mobile_camera_with_computer_using_python/blob/master/using_USB/camera_input_from_Android.ipynb

	# Replace the URL with your own IPwebcam shot.jpg IP:port
	#url= 'http://10.2.0.100:8080//shot.jpg' #
	url = 'http://192.168.43.1:8080//shot.jpg' #//shot.jpg' #http://192.168.1.137:8080//shot.jpg
	...
	timeTb = tw.TimeWatch(10)

	counters = ['ImgProc','FrameHandle','ImgManip','ArucoDetection','WebRequest','ImgPred','ImgPlot']

	timeTb.createCounter(counters)
	...
	while not rospy.is_shutdown() and N < 100:
		...
		dtime = t01 - t00
		timeTb.update('WebRequest',dtime)
		...
		dtime = tf - t0
		timeTb.update('FrameHandle',dtime)
		...
		dtime = tf - t0
		timeTb.update('ArucoDetection',dtime)
		...
		if center_current != None and ponto_atual != 0 and arucopixel != None and aruco_center != 2 and centers[ponto_atual-3] != None:
			flag_pose = 0
			...
			if (ponto_atual) % 1 == 0:
				...
				print(centers)
				print(center_current, center_previous, arucosize, arucopixel, deltatime, max_x_right)
				...
				elif (center_current[0]- center_previous[0]) != 0 and (center_current[1]- center_previous[1]) != 0:
					flag_pose = 1
	...
	timeTb.plotTimes(['ImgProc','FrameHandle','WebRequest','ArucoDetection','ImgManip'])
	timeTb.plotTimes(['ImgProc','ImgPred','ImgPlot'])
